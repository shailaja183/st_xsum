...
class MainView(Component):
    ...
    def html(self):
        ...
        for summary_idx, summary in enumerate(self.summaries):
            ...
            #print(self.hallu)
            #print(type(self.hallu)) 
            #print(self.reason)
            #print(type(self.reason))   
            tok_range = self.hallu  
            tok_reason = self.reason
            token_elements2 = []
            for token_idx, token in enumerate(summary):
                if token_idx in tok_range:
                    if tok_reason[token_idx]!="":
                        el = span(style=styles(background_color="yellow",opacity=0.5))(util.annotation(*(token.text,tok_reason[token_idx])))
                    else:    
                        el = span(style=styles(background_color="yellow",opacity=0.5))(token.text+" ")
                else:
                    el = token.text+" "   
                token_elements2.append(el)

            '''
            x = ( "\" ", ("Maverick","Adj"), " \" ", "is a word that seems to", ("follow", "Verb"), " Craig Venter around ." )
            #x = util.annotation("apple", "fruit", background="#FF0", border="1px dashed red")
            
            
            token_elements3 = []
            for arg in self.reason:
                if arg=="":
                    token_elements3.append(arg)
                if isinstance(arg, str):
                    token_elements3.append(arg)
                elif isinstance(arg, tuple):
                    token_elements3.append(util.annotation(*arg))
                


            #div(_class="name")("<hr />"),
            #div(_class="name")("Reasoning Annotation (Our work)"),
            #div(_class="content")(token_elements3)
            '''
            ...
            summary_items.append(
                div(
                    **{"class": ' '.join(classes), "data-index": summary_idx}
                    ...
                    div(_class="name")("<hr />"),
                    div(_class="name")("Hallucination (Maynez, 2020) + Reasoning (Ours)"),
                    div(_class="content")(token_elements2)
                )
            )
        ...
        annotation_key = \
            """
              <ul class="annotation-key">
                ...
                <li id="option-hallu" class="option selected">
                    <span class="annotation-key-hallu" style="background-color:#ffff00;">Hallucination Spans</span>
                </li>
            </ul>
            """
...
if __name__ == "__main__":
    ...
    tokens = [
        ...
        htbuilder.span(style=styles(background_color="yellow"))("brown"),
        "fox",
        "jumps"
    ]
    spans = [
