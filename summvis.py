...
class Instance():
    def __init__(self, id_, document, reference, preds, hallu, reason, data=None):
        self.id = id_
...
def retrieve(dataset, index, filename=None):
    ...
    try:
        hallu = data['hallu:reference']
    except KeyError:
        st.error("no hallucination annotations found")

    try:
        reason = data['reason:reference']
    except KeyError:
        st.error("no reasoning annotations found")
...
def show_main(example):
    ...
    #print(example.hallu)
    #print(hallu)
    hallu = example.data[f'hallu:reference']
    reason = example.data[f'reason:reference']
